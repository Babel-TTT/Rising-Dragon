...
NMLC ?= nmlc -c
GCC  ?= gcc
PROJECT ?= military-pack
INDEX_FILE ?= $(PROJECT).pnml
OUTPUT_NML ?= $(PROJECT).nml
OUTPUT_GRF ?= $(PROJECT).grf
...
bundle: code FORCE
	@echo "Bundling"
	@cp $(OUTPUT_GRF) docs/
	@cp LICENSE docs/license.txt; cp README.md docs/readme.txt; cp CHANGELOG.md docs/changelog.txt
	@cd docs && tar -cf ../$(PROJECT)-$(REPO_VERSION_STRING).tar $(DOC_FILES) $(OUTPUT_GRF)
	@rm docs/$(OUTPUT_GRF); rm docs/license.txt; rm docs/readme.txt; rm docs/changelog.txt
